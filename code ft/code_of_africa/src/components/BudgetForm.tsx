...
export default function BudgetForm() {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof formSchema>> = async (values) => {
    ...
    try {
      const response = await fetch('https://your-api-endpoint.com/api/budgets', {
        method: 'POST',
        headers: {
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          <Button type="submit" className="w-full" disabled={isSubmitting}>
            {isSubmitting ? "Sending..." : "Create Budget"}
          </Button>
        </form>
      </Form>
      {submitSuccess && <p className="mt-4 text-green-600 text-center">Budget created successfully!</p>}
      {submitError && <p className="mt-4 text-red-600 text-center">Error creating budget. Please try again.</p>}
    </div>
  );
}
